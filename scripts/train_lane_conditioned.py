"""Train a lane-conditioned multimodal decoder over a frozen scene encoder."""
...
def _evaluate(
    model: LaneConditionedForecaster,
    loader: DataLoader,
    device: torch.device,
...
def _save_curves(rows: list[dict[str, float]], output: Path) -> None:
    ...
    columns = (
        ...
        ("val_miss_rate", "Validation MissRate@K", "Rate"),
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--init-checkpoint", type=Path, default=Path("checkpoints/multimodal_20k.pt")
    )
    parser.add_argument(
    ...
    parser.add_argument("--num-lane-modes", type=int, default=4)
    ...
    parser.add_argument("--run-name", default="lane-conditioned")
    parser.add_argument(
        "--output", type=Path, default=Path("checkpoints/lane_conditioned.pt")
    )
    ...
    if (
        ...
        or args.num_neighbors < 1
        or args.num_modes < 1
        or args.num_lane_modes < 0
        or args.num_lane_modes > args.num_modes
        or args.classification_weight < 0
        or args.diversity_weight < 0
        or args.diversity_margin < 0
        or args.miss_threshold < 0
        or args.num_workers < 0
    ):
        parser.error("invalid dimensions, epochs, loss weights, thresholds, or worker count")
    ...
    if not args.init_checkpoint.is_file():
        parser.error(f"initialization checkpoint not found: {args.init_checkpoint}")
    ...
    initial_config = initial["model_config"]
    model_config = {
        "input_dim": int(initial_config["input_dim"]),
        "neighbor_dim": int(initial_config["neighbor_dim"]),
        "lane_dim": int(initial_config["lane_dim"]),
        "hidden_dim": args.hidden_dim,
        "future_steps": int(initial_config["future_steps"]),
        "max_neighbors": args.num_neighbors,
        "max_lanes": int(initial_config["max_lanes"]),
        "points_per_lane": int(initial_config["points_per_lane"]),
        "num_modes": args.num_modes,
        "num_lane_modes": args.num_lane_modes,
        "sampling_interval": 0.1,
    }
    if model_config["hidden_dim"] != int(initial_config["hidden_dim"]):
        parser.error("hidden-dim must match the initialized encoder checkpoint")
    if model_config["max_neighbors"] != int(initial_config["max_neighbors"]):
        parser.error("num-neighbors must match the initialized encoder checkpoint")
    ...
    expected_encoder_keys = {
        key for key in target_state if key.startswith(encoder_prefixes)
    }
    if set(pretrained_encoder) != expected_encoder_keys:
        missing = sorted(expected_encoder_keys - set(pretrained_encoder))
        parser.error(f"initial checkpoint is missing compatible encoder tensors: {missing[:5]}")
    ...
    pair_indices = torch.triu_indices(
        args.num_modes, args.num_modes, offset=1, device=device
    )
    ...
    print(
        f"Training scenarios: {len(train_data)}; validation scenarios: {len(val_data)}",
        flush=True,
    )
    print(
        f"Warm-started frozen scene encoder from {args.init_checkpoint}; "
        f"modes={args.num_modes} ({args.num_lane_modes} lane + "
        f"{args.num_modes - args.num_lane_modes} free)",
        flush=True,
    )
            ...
            for batch in train_loader:
                ...
                if args.num_modes > 1:
                    ...
                    distances = torch.linalg.vector_norm(
                        endpoints[:, pair_indices[0]] - endpoints[:, pair_indices[1]],
                        dim=-1,
                    )
                    diversity_loss = torch.relu(
                        args.diversity_margin - distances
                    ).square().mean()
                else:
                ...
                epoch_losses.append(
                    [
                        float(loss.detach()),
                        float(trajectory_loss.detach()),
                        float(classification_loss.detach()),
                        float(diversity_loss.detach()),
                    ]
                )

            metrics = _evaluate(model, val_loader, device, args.miss_threshold)
